...
function doRectanglesOverlap(rect1, rect2) {
  const intersectionX1 = Math.max(rect1.left, rect2.left);
  const intersectionX2 = Math.min(rect1.left + rect1.width, rect2.left + rect2.width);
  if (intersectionX2 < intersectionX1) {
    return false;
  }

  const intersectionY1 = Math.max(rect1.top, rect2.top);
  const intersectionY2 = Math.min(rect1.top + rect1.height, rect2.top + rect2.height);
...
function findFirstSingleChar(str) {
  return str.split('').reduce((prev, cur, index, arr) => {
    if (!prev && cur !== '' && arr.filter((x) => x === cur).length === 1) {
      return cur;
    }
    return prev;
  }, null);
...
function isCreditCardNumber(ccn) {
  ...
  const last = arr.splice(0, 1)[0];
  const sum = arr.reduce((acc, val, i) => {
    let res = i % 2 !== 0 ? val : val * 2;
    if (res > 9) {
      res -= 9;
    }
    return acc + res;
...
function isBracketsBalanced(str) {
  ...
  return str.split('').reduce((acc, val) => {
    if (acc) {
      const bracketsIndex = brackets.indexOf(val);
...
function getMatrixProduct(m1, m2) {
  ...
  return result.map((row, i) => row
    .map((val, j) => m1[i].reduce((sum, elm, k) => sum + (elm * m2[k][j]), 0)));
}
